...
8. **UI выбора команд**: флаги стран (`images_countries/<id>.png`, из скрейпера, маппинг по
   имени страны), убран «пронос» иконок в углу при смене страны (иконки позиционируются в
   `Redraw`, а не при `AddEntry`), панель игрока 2 строится лениво при открытии.
