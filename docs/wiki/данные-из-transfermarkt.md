...
плоский JSON на TM-id (`data/canon/`, см. [[пайплайн-данных]]); конвертеры производят из канона
GF SQLite и Flutter-assets.
...
|---|---|
| `players.tm_id`, `teams.tm_id`, `leagues.tm_id`, `countries.tm_id` | **внешние стабильные ключи** (схема БД v2, 2026-09-09): TM-id игрока/клуба/лиги/страны из `data/full/*.json` (сборные — `team["id"]`). NULL — только у синтетических строк («International», «National Teams»). Внутренние rowid — ключи, которыми читает игра; `tm_id` — для внешнего матчинга (патчи, сеть, карьера). Матчинг в конвейере — только по `tm_id` |
...
5. **Фото игроков**: ассеты — портретные вырезки Transfermarkt (фон удалён, палитра 128,
   160×208, ~10 КБ/фото). Генератор `tm-gf-face-generator/export_cutouts.py`
   (`--matting mediapipe`) читает `data/images/faces/<tm_id>.jpg` и пишет
   `data/images/faces_cutout/<tm_id>.png`; плейсхолдеры TM (белый силуэт) дают пустую маску →
   файл не пишется. Импорт (`builders/files.py::copy_faces`) кладёт вырезки в
   `databases/default/faces/<tm_id>.png`. Игра читает портрет по `players.tm_id` из
   `databases/default/faces/<tm_id>.png` (плейсхолдер `media/menu/player_placeholder.png`,
   если файла нет; `tm_id` едет и в сетевом setup, протокол v13). `copy_data_post_build`
   копирует `faces/` в сборку только для релизной конфигурации
   (`tools/copy_data.cmake`, флаг `COPY_FACES`); dev-сборка их исключает (несколько PNG
   можно положить вручную). В git лица не хранятся — для сборки из исходников они лежат
   отдельным релизным бандлом `GameplayFootball-faces-<ver>.zip` (см. [[релиз]], README).
   Копирование фото **кэшируется**: stamp-файл
   `<build>/.faces_copied` хранит время исходного каталога, поэтому ~63k PNG копируются
   один раз, а повторно — только если исходник изменился (ре-импорт); сбросить — удалить
   `<build>/databases/default/faces` и `<build>/.faces_copied`. Вырезки — палитровые PNG,
   SDL_image грузит их как
   `SDL_PIXELFORMAT_INDEX8`; `Surface::SetData` (`src/scene/resources/surface.cpp`)
   конвертирует индексированные поверхности в RGBA32 перед загрузкой в GL — иначе
   `GetGLPixelFormatFromSurface` отдаёт `GL_RGB` на 1 байт/пиксель и текстура «едет»
   (индексы палитры читаются как цвет).
...
8. **UI выбора команд**: флаги стран (`images_countries/<tm_id>.png`) — **единый формат 512×512**,
   флаг по центру холста с сохранением собственных пропорций (прозрачные поля). Источник —
   **скрейпер** (`flags.py` → `data/images/country_flags/<tm_id>.png`, копируются импортом;
   flagcdn.com `w1280` для ISO-стран + Wikimedia Commons для England/Scotland/Wales/Northern
   Ireland, на flagcdn их нет). Ключ — TM-id страны (`countries.tm_id`), стабильный между
   пересборками БД. Квадратный холст повторяет квадратную иконку карусели (виртуальный
   aspect меню 5:4 → 8%×10% = 1:1).
...
- **Схема парсит int из строк**: `parse_str_to_int` в `schemas/base.py` ожидает строку, int уже
  передавать нельзя.
- **Дубли клубов в источнике (закрыто 2026-09-09)**: лиги, которые TM разбивает на фазы
  Apertura/Clausura, давали один клуб (тот же `id`) 2–3× в `data/full/clubs.json`. Скрейпер
  починен (`build_pilot_json.py` перезаписывает клуб по id внутри лиги вместо аппенда), файл
  пересобран из кэша без дублей; дедуп в импорте убран.