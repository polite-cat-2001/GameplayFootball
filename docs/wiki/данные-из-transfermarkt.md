...
- **Дубли клубов в источнике**: `data/full/clubs.json` содержит один и тот же клуб (тот же
  ...
  «2. Division A (Phase 1)» ×2). Импорт дедуплицирует **по TM-id клуба внутри лиги**
  (`tm-gf-import/builders/teams.py`, `players.py`); игроки дубля не вставляются повторно.
  Скрейперу стоит не аппендить уже присутствующий клуб в лигу.